...
import 주문확인Repository 
import Hateoas 

bp = Blueprint('주문확인', __name__, url_prefix='/주문확인')

@bp.route("", methods=["POST"])
def Post():
    response = 주문확인Repository.Create(request)
...
def Get():
    response = 주문확인Repository.Read(request)
...
def Get_By_Id(id: int):
    response = 주문확인Repository.Read_by_id(id)
...
def Update(id: int):
    ...
    response = 주문확인Repository.Update(id, request)
...
def Delete(id: int):
    response = 주문확인Repository.Delete(id)
    if response == None:
